...
async function run() {
   ...
   const langs = await fs.readJSON('packages/shikix/src/assets/langs.json') as { id: string, name?: string, aliases?: string[] }[]
   ...
   const themes = await fs.readJSON('packages/shikix/src/assets/themes.json') as { id: string, name?: string }[]
