...
Inherited from [`shiki`](https://github.com/shikijs/shiki/blob/main/docs/languages.md#supporting-your-own-languages-with-shiki), here are all the languages bundled in `shikix`.
...
Since `shikix` is environment agnostic, that means we don't have access to the file system. That means the `path` property `shiki` supports is not available in `shikix`. Instead, you need to read them in yourself and pass the object. For example:
