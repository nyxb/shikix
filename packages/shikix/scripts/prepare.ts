...
const customThemes = {
   'lumos-dark': `__(() => import('./lumos/lumos-dark.json')) as unknown as DynamicThemeReg__`,
   'lumos-light': `__(() => import('./lumos/lumos-light.json')) as unknown as DynamicThemeReg__`,
}

const allThemes = {
   ...Object.fromEntries(BUNDLED_THEMES.sort().map(i => [i, `__(() => import('shiki/themes/${i}.json')) as unknown as DynamicThemeReg__`])),
   ...customThemes,
}
...
export const bundledThemes = ${JSON.stringify(allThemes, null, 2).replace(/"__|__"/g, '')}
`,
   'utf-8',
)
...
await fs.writeJSON(
   ...
   [...BUNDLED_THEMES, ...Object.keys(customThemes)].map(i => ({
      id: i,
   })),
   { spaces: 2 },
)
