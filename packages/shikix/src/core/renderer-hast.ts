...
export function codeToHast(
   ...
   if ('themes' in options) {
      ...
      if (themes.length === 0)
         throw new Error('[shikix] `themes` option must not be empty')
      ...
      if (defaultColor && !themeTokens.find(t => t[0] === defaultColor))
         throw new Error(`[shikix] \`themes\` option must contain the defaultColor key \`${defaultColor}\``)
      ...
      throw new Error('[shikix] Invalid options, either `theme` or `themes` must be provided')
   }
...
export function tokensToHast(
   ...
   if (options.transforms) {
      ...
      console.warn('[shikix] `transforms` option is deprecated, use `transformers` instead')
   }
