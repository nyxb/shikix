...
export class Registry extends TextMateRegistry {
      ...
      if (this.alias[name]) {
         ...
         while (this.alias[name]) {
            ...
            if (resolved.has(name))
               throw new Error(`[shikix] Circular alias \`${Array.from(resolved).join(' -> ')} -> ${name}\``)
            resolved.add(name)
         }
      }
   ...
   public async loadLanguages(langs: LanguageRegistration[]) {
      ...
      if (missingLangs.length) {
         ...
         throw new Error(`[shikix] Missing languages ${missingLangs.map(([name]) => `\`${name}\``).join(', ')}, required by ${dependents.map(([name]) => `\`${name}\``).join(', ')}`)
      }
