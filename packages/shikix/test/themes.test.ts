...
describe('syncThemesTokenization', () => {
   ...
   it('two themes', async () => {
      const lines1 = await codeToThemedTokens('console.log("hello")', { lang: 'js', theme: 'lumos-dark', includeExplanation: true })
...
`
   ...
   it('multiple themes', async () => {
      ...
      const code = await codeToHtml('import * as Shiki from "shikix"', {
         lang: 'js',
         themes,
         cssVariablePrefix: '--s-',
      })
...
describe('errors', () => {
   it('throws on empty theme', async () => {
      expect(() => codeToHtml('console.log("hello")', {
         ...
         .rejects
         .toThrowErrorMatchingInlineSnapshot(`[Error: [shikix] \`themes\` option must not be empty]`)
   })
   ...
   it('throws on missing default color', async () => {
      expect(() => codeToHtml('console.log("hello")', {
         ...
         .rejects
         .toThrowErrorMatchingInlineSnapshot(`[Error: [shikix] \`themes\` option must contain the defaultColor key \`light\`]`)
      ...
      expect(() => codeToHtml('console.log("hello")', {
         ...
         .rejects
         .toThrowErrorMatchingInlineSnapshot(`[Error: [shikix] \`themes\` option must contain the defaultColor key \`dark\`]`)
   })
