...
describe('should', () => {
   ...
   it('dynamic load theme and lang', async () => {
      ...
      await shiki.loadTheme(() => import('../dist/themes/lumos-light.mjs').then(m => m.default))
      ...
      expect(shiki.codeToHtml('print 1', { lang: 'python', theme: 'lumos-light' }))
         .toMatchInlineSnapshot(`"<pre class="shiki lumos-light" style="background-color:#ffffff;color:#393a34" tabindex="0"><code><span class="line"><span style="color:#998418">print</span><span style="color:#2F798A"> 1</span></span></code></pre>"`)
   })
...
describe('errors', () => {
   it('throw on invalid theme', async () => {
      ...
      await expect(() => shiki.codeToHtml('console.log("Hi")', { lang: 'javascript', theme: 'invalid' }))
         .toThrowErrorMatchingInlineSnapshot(`[Error: [shikix] Theme \`invalid\` not found, you may need to load it first]`)
   })
   ...
   it('throw on invalid lang', async () => {
      ...
      await expect(() => shiki.codeToHtml('console.log("Hi")', { lang: 'abc', theme: 'nord' }))
         .toThrowErrorMatchingInlineSnapshot(`[Error: [shikix] Language \`abc\` not found, you may need to load it first]`)
   })
   ...
   it('throw on circular alias', async () => {
      ...
      await expect(() => shiki.codeToHtml('console.log("Hi")', { lang: 'mylang', theme: 'nord' }))
         .toThrowErrorMatchingInlineSnapshot(`[Error: [shikix] Circular alias \`mylang -> mylang2 -> mylang\`]`)
   })
})
